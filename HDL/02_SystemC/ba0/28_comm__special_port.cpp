...
// Backup from: https://www.learnsystemc.com/basic/specialized_port
...
Other than using the basic sc_port class to declare ports, there are various specialized port classes to use with different channel types, or provide additional functions:
   ...
   2. sc_fifo_in:  a specialized port class for use when reading from a fifo.
   3. sc_fifo_out: a specialized port class for use when writing to a fifo.
   4. sc_in<bool> and sc_in<sc_dt::sc_logic>: value_changed(), pos(), neg()
   5. sc_inout:    a specialized port class for use with signals: value_changed(), initialize()
   6. sc_inout<bool> and sc_inout<sc_dt::sc_logic> are specialized port classes that provide additional member functions for 
            two-valued signals: value_changed(), initialize(), pos(), neg()
   7. sc_out:      derived from class sc_inout and is identical to class sc_inout except for differences inherent in it being 
            a derived class, for example, constructors and assignment operators.
   8. sc_in_resolved:    is a specialized port class for use with resolved signals. 
            It is similar to sc_in<sc_dt::sc_logic> from which it is derived. 
            The only difference is that a port of class sc_in_resolved shall be bound to a channel of class sc_signal_resolved, 
            whereas a port of class sc_in<sc_dt::sc_logic> may be bound to a sc_signal<sc_dt::sc_logic,WRITER_POLICY> or class sc_signal_resolved.
   9. sc_inout_resolved: is a specialized port class for use with resolved signals. 
            It is similar in behavior to port class sc_inout<sc_dt::sc_logic> from which it is derived. 
  ...
  10. sc_out_resolved: is derived from class sc_inout_resolved, and it is identical to class sc_inout_resolved except for 
            differences inherent in it being a derived class, for example, constructors and assignment operators.
  ...
  13. sc_out_rv:       is derived from class sc_inout_rv, and it is identical to class sc_inout_rv except 
            for differences inherent in it being a derived class, for example, constructors and assignment operators. 
...
An sc_port<sc_signal_inout_if<bool>> has access to these additional member functions provided by signal<bool> channel:
   ...
   9. negedge_event() // event for value changes from true to false
